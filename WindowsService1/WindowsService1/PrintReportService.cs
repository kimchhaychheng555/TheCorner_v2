...
namespace WindowsService1
{
    ...
    {
        public void printReceipt(SaleModel sale, string exchange, string printerName, string fileName)
        {
            string fileLoadPath = AppDomain.CurrentDomain.BaseDirectory + @"Report\" + fileName;
            try
            {
                ModuleService.alertTelegram(string.Format("PrintReceipt>Pending SaleID = {0}", sale.id));
                ...
                ModuleService.alertTelegram(string.Format("PrintReceipt>Print Success SaleID = {0}", sale.id));
            }
            catch (Exception ex)
            {
                ModuleService.alertTelegram(string.Format("Catch>Print Receipt>{0}", ex.Message.ToString()));
            }
        }

        private DataTable getDataTableSale(SaleModel sale)
        {
        ...
        private DataTable getDataTableSaleProduct(List<SaleProductModel> saleProducts)
        {
            DataTable dataSale = new DataTable();
